...
contract ForkRegisterTokenTest is Utils {
    ...
    function setUp() public {
        ...
        vm.prank(lrtSquare.governor());
        lrtSquare.registerToken(newToken, tokenPositionWeightLimit);
        ...
        vm.prank(lrtSquare.governor());
        lrtSquare.setDepositors(depositors, shouldWhitelist);
    }
    ...
    function test_CanDeposit() public {
        ...
        uint256 aliceBalBefore = lrtSquare.balanceOf(alice);

        vm.startPrank(alice);
        IERC20(newToken).forceApprove(address(lrtSquare), amountToDeposit);
        lrtSquare.deposit(tokens, amounts, alice);
        vm.stopPrank();

        uint256 aliceBalAfter = lrtSquare.balanceOf(alice);
